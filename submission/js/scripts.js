        ...
        if (getByTitle.length == 0) {
            ...
            if (getByAuthor.length == 0) {
                ...
                if (getByYear.length == 0) {
                    alert(`Tidak ditemukan data dengan judul: ${searchValue.value}`)
                }else{
...
function deleteBook(id) {
    let confirmation = confirm("Apakah Yakin akan Dihapus?")
    ...
    if (confirmation == true) {
        ...
        localStorage.setItem(localStorageKey,JSON.stringify(bookData))
        showData(getData())
        alert(`Buku ${bookDataDetail[0].title} telah dihapus`)
    }else{
        return 0
    }
}